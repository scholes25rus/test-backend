...
class BudgetEntity(id: EntityID<Int>) : IntEntity(id) {
    ...
    fun toResponse() = BudgetResponse(year, month, amount, type, author?.fullname, author?.createdDate.toString())
    fun toCreateResponse() = BudgetRecord(year, month, amount, type, author?.id?.value)
